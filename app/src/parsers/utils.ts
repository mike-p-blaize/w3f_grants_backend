...
const IGNORE_LINK_TITLES = ['invoice form :pencil:', 'Levels'];
...
export function findGrantMatch(key, grants) {
  const targets = grants.map((x) =>
    cleanFileName(x.toLowerCase().replace('.md', '')),
  );
...
export function parseLinks(text) {
  ...
  for (const match of matches) {
    ...
    const ignore =
      IGNORE_LINK_TITLES.includes(link.title) ||
      IGNORE_LINK_URLS.includes(link.url);
    if (!ignore) {
      links.push(link);
    }
  }
