...
export class DataService {
  ...
  constructor() {
    try {
      this.db = {};
      const pathDB = pathNode.resolve(
        process.env.TMP_DATA_DIRECTORY + '/db.json',
      );
      if (!fs.existsSync(pathDB)) {
        this.fullSync().then(() => this.loadFromFile());
      } else {
        this.loadFromFile();
      }
    } catch (e) {
  ...
  async gitClone(): Promise<object> {
    ...
    if (!fs.existsSync(tmp_data_folder)) {
      fs.mkdirSync(tmp_data_folder, { recursive: true });
    }
    ...
    await Shell.exec(
      'git clone git@github.com:w3f/Grant-Milestone-Delivery.git',
      { silent: true },
    );
    await Shell.cd('..');
    return {
  ...
  async parseFolderData(
    ...
    for (let i = 0; i < allFiles; i++) {
      ...
      let ok = fileName.endsWith('.md') || fileName.endsWith('.MD');
      ...
      if (ok) {
        ...
        const format =
          ' --date=iso-strict --pretty=format:\'{%n  "commit": "%H",%n  "author": "%aN <%aE>",%n  "date": "%ad",%n  "message": "%f"%n },%n  \' ';

        const log = Shell.exec(
          `git log --first-parent master ${format} "${fileName}"`,
          { silent: true },
        ).stdout;
    ...
    Shell.cd(pathNode.join('..', '..', '..'));
    return [ans, numFilesProcessed, numWarnings];
  }

  async parseApplications(): Promise<object> {
    const folder = pathNode.join(
      process.env.TMP_DATA_DIRECTORY,
      'Grants-Program',
      'applications',
    );
    ...
    const [data, numFilesProcessed, numWarnings] = await this.parseFolderData(
      folder,
      excludeFiles,
      parseFunction,
    );
  ...
  async parseDeliveries(): Promise<object> {
    ...
    const excludeFiles = [
      '.delivery_testing.md',
      'milestone-delivery-template.md',
    ];
  ...
  async parseEvaluations(): Promise<object> {
    const folder =
      process.env.TMP_DATA_DIRECTORY + '/Grant-Milestone-Delivery/evaluations';
  ...
  buildMainDataset() {
    for (const i in this.db.applications) {
